<template>
  ...
  <div class="justify-between px-[10px] shadow-2xl bg-[#184BCE] h-screen">
    <div class="py-[10px] border-b border-gray-500">
      <router-link
        to="/dashboard"
        class="inline-flex items-center text-center w-full text-white text-[18px] font-bold py-[6px] px-[24px] hover:bg-white hover:text-black rounded-[6px]"
    ...
    <div class="">
      ...
      <div class="py-[10px] border-b border-gray-500">
        <router-link to="/storage">
          <button
            ...
            class="inline-flex items-center text-center w-full text-white text-[18px] font-bold py-[6px] px-[24px] hover:bg-white hover:text-black rounded-[6px]"
          >
            <span class="pr-[10px]">ห้องเก็บของ</span>
            <svg
              class="w-4 h-4 items-end"
      ...
      <div v-show="isOpen" class="py-[10px] border-b border-gray-500">
        <button
          :class="{ activeD: isTextActive(1) }"
          @click="handleTextClick(1), (openShow = !openShow)"
          class="inline-flex items-center text-center w-full text-[#7098FF] text-[18px] font-bold py-[6px] px-[24px] pl-[30px] hover:text-white"
        >
          <span class="pr-[10px]">อุปกรณ์คอมพิวเตอร์</span>
          <svg
            class="w-4 h-4 items-end"
            :class="{ 'transform rotate-180': openShow }"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
        ...
        <div v-show="openShow">
          <div class="py-[10px]">
            <router-link
              to="/back"
              class="inline-flex items-center text-center w-full text-[#7098FF] text-[18px] font-bold py-[6px] px-[24px] pl-[30px] hover:text-white"
              :class="{ activeD: isTextActive(2) }"
              @click="handleTextClick(2)"
            >
              - เมาส์
            </router-link>
          </div>
          ...
          <div class="py-[10px]">
            <router-link
              to="/back"
              class="inline-flex items-center text-center w-full text-[#7098FF] text-[18px] font-bold py-[6px] px-[24px] pl-[30px] hover:text-white"
              :class="{ activeD: isTextActive(3) }"
              @click="handleTextClick(3)"
            >
              - คีย์บอร์ด
            </router-link>
          </div>
          ...
          <div class="py-[10px]">
            <router-link
              to="/back"
              class="inline-flex items-center text-center w-full text-[#7098FF] text-[18px] font-bold py-[6px] px-[24px] pl-[30px] hover:text-white"
              :class="{ activeD: isTextActive(4) }"
              @click="handleTextClick(4)"
            >
              - จอ
            </router-link>
          </div>
          ...
          <div class="py-[10px]">
            <router-link
              to="/back"
              class="inline-flex items-center text-center w-full text-[#7098FF] text-[18px] font-bold py-[6px] px-[24px] pl-[30px] hover:text-white"
              :class="{ activeD: isTextActive(5) }"
              @click="handleTextClick(5)"
            >
              - คอม
            </router-link>
          </div>
          ...
          <div class="py-[10px]">
            <router-link
              to="/back"
              class="inline-flex items-center text-center w-full text-[#7098FF] text-[18px] font-bold py-[6px] px-[24px] pl-[30px] hover:text-white"
              :class="{ activeD: isTextActive(6) }"
              @click="handleTextClick(6)"
            >
              - สายจอ
            </router-link>
          </div>
          ...
          <div class="py-[10px]">
            <router-link
              to="/back"
              class="inline-flex items-center text-center w-full text-[#7098FF] text-[18px] font-bold py-[6px] px-[24px] pl-[30px] hover:text-white"
              :class="{ activeD: isTextActive(7) }"
              @click="handleTextClick(7)"
            >
              - สายพาวเวอร์
            </router-link>
          </div>
...
const handleTextClick = (textNumber) => {
  state.activeText = textNumber;
};

const isTextActive = (textNumber) => {
  return state.activeText === textNumber;
};
</script>
