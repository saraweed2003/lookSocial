<template>
  ...
  <div class="px-[90px] pt-[40px] font-Kanit">
    <div class="grid grid-cols-8 gap-[50px]">
      ...
      <div class="col-span-2 drop-shadow-md" v-for="items in testapi" :key="items.id">
        <div class="bg-[#FFFFFF] h-[400px] rounded-[8px]">
          ...
          >
            <template v-if="!showInput">
              <router-link
                v-for="(nameItem, index) in testapi"
                :key="index"
...
onMounted(async () => {
  try {
    ...
    testapi.value = response.data.data;
    console.log("123", testapi.value);
  } catch (error) {
